...
?>
		...
		<div class="search-forms">    
			<form action="" method="get" class="juristic2-serach xs-search-group">
				...
				<div class="input-group">
					...
					<button class="input-group-btn search-button search-button" type="submit"><i class="icon icon-search"></i></button>
				</div>
					<tbody>
						<tr>
							<td style="width: 100%;">
				...
				<div class="search-advanced">
					<div class="search-advanced-result">
						<?if(is_object($arResult["NAV_RESULT"])):?>
							<div class="search-result pl-2"><?echo GetMessage("CT_BSP_FOUND")?>: <?echo $arResult["NAV_RESULT"]->SelectedRowsCount()?></div>
						<?endif;?>
						<?
						$arWhere = array();
					...
					<?if($arParams["SHOW_WHERE"] || $arParams["SHOW_WHEN"]):?>
						...
						<div class="search-advanced-filter">
							<a href="#" onclick="return switch_search_params()"><?echo GetMessage('CT_BSP_ADVANCED_SEARCH')?></a>
						</div>
					</div>
					<div id="search_params" class="search-filter" style="display:<?echo $arResult["REQUEST"]["FROM"] || $arResult["REQUEST"]["TO"] || $arResult["REQUEST"]["WHERE"]? 'block': 'none'?>">
						...
						<table class="search-filter" cellspacing="0">
							<tbody>
							<?if($arParams["SHOW_WHERE"]):?>
							<tr>
								<td class="search-filter-name">
									<?echo GetMessage("CT_BSP_WHERE")?>
								</td>
