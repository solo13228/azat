...
<section class="services-section-two style-two pt-0">
	...
	<div class="services-block tab-btn">
						<div class="inner-box" data_id="0">
							<div class="content">
								<h3><a data_id="0"><?=GetMessage("ALLC")?></a></h3>
							</div>
						</div>
					</div>
		<?$i = 0;?>
...
<?foreach($arResult["ITEMS"] as $arItem):?>
			<?
				...
				$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
			?>
			<div class="services-block tab-item col-lg-4 col-md-6 col-sm-6 section_0">
				<div class="inner-box active" data_id="0">
					<div class="content">
						<div class="icon-box col-3 mr-3 m-0">
	 						<span class="icon m-0">
	 							<img src="<?=$arItem["DISPLAY_PROPERTIES"]["SERVICES_ICON"]["FILE_VALUE"]["SRC"]?>" alt="">
	 							
	 						</span>
	 						<span class="icon d-none <?=$icons_pull[$i]?>"></span>
						</div>
						<h3 style="margin-left: 35px;" ><a maxlength="5" href="<?=$arItem['DETAIL_PAGE_URL'];?>"><?=$arItem["NAME"] ?></a></h3>
					</div>
				</div>
			</div>
	<?endforeach;?>
			...
			<div class="services-block tab-item col-lg-4 col-md-6 col-sm-6 section_<?=$arSection['ID'];?>">
				<div class="inner-box">
					<div class="content">
						...
						?>
						<h3 style="margin-left: 33px;"><a href="<?=$arProps['DETAIL_PAGE_URL'];?>"><?=$title_text;?></a></h3>
					</div>
				</div>
			</div>
						<?}?>
...
<script type="text/javascript">
	$('.services-section-two .services-block.tab-btn .inner-box[data_id="0"]').addClass("active");
	$(".services-section-two .services-block.tab-item.section_0").addClass("active");
